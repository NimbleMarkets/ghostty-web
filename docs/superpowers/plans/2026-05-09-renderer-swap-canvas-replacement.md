...
4. To force a *runtime* device loss, you need to navigate while WebGPU is enabled, then trigger via Chrome's GPU-process kill (chrome://gpu) or use the WebGPU `requestDevice().lost` mechanism if exposed
